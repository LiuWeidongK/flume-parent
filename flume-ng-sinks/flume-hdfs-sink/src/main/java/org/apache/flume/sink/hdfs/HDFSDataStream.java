...
public class HDFSDataStream extends AbstractHDFSWriter {
  ...
  private final DecimalFormat df = new DecimalFormat("#.00");   // format
  ...
  public void configure(Context context) {
    ...
    logger.info(
        "Serializer = " + serializerType + ", UseRawLocalFileSystem = " + useRawLocalFileSystem);
  }
...
//    logger.info(" - TEST ME - Use data stream and serializer is " + serializer.getClass()
//        .getCanonicalName());

//    long start = System.currentTimeMillis();    // start time

    // FIXME: 2019/5/20 Specific business logic
    serializer.write(e);

//    double time = (System.currentTimeMillis() - start) / 1000.0;    // use time
//    double size = e.getBody().length / 1024.0 / 1024.0; // file size
//    double avg = size / time;   // average speed
//
//    logger.info(
//        " - TEST ME - Gz file size : " + df.format(size) + " MB, Use time : " + df.format(time)
//            + " s, Average speed : " + df.format(avg));
  }
